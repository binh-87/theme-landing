        ...
        xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                if(isInIFrame){
                    ...
                    html +=         '<a class="btn js-purchase__add-to-cart e-btn--3d -color-primary -size-m -width-full" href="' + linkRef + '"><i class="e-icon -icon-cart -margin-right"></i> <strong>Add to Cart</strong></a>';
                    html +=         '</div>';
...
jQuery(document).ready(function($) {
        ...
        xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                for(i; i > 0; i--) {
                    ...
                    html += '<a target="_blank" href="' + linkRef + '"><img alt="' + item[i].item + '" src="' + placeholder + '" data-src="' + item[i].live_preview_url + '" title="' + item[i].item + '" /></a>';
                    html += '</div>';
    ...
    (function($){
        "use strict"; // Start of use strict
       var featureditem = $('footer');
       var ref_url = 'themeforest';
       var host_name = '//alothemes.com/';
       $.ajax({url: host_name +'graphql',
          ...
          success: function(result) {
             // console.log(JSON.stringify(result));
                ...
                html    += '<p class="section-des">More our services from us. Please take a look from our services below!</p>';
                html    += '<ul class="magicslider magepow">';
                ...
                $.each( items, function( key, item ) {
                    ...
                    html += '<div class="item-box">';
                    html += '<div class="img">';
                    var linkRef =  host_name + item.url_key + '.html?ref=' +  ref_url;
                    html += '<a target="_blank" href="' + linkRef + '"><img alt="' + item.image.label + '" src="' + placeholder + '" data-src="' + item.image.url + '" title="' + item.image.label + '" /></a>';
                    html += '</div>';
              ...
              $('.magicslider.magepow').slick(slidecfg);
          }
       });
    })($); // End of use strict
       ...
       $.ajax({url: host_name +'graphql',
          ...
          success: function(result) {
             // console.log(JSON.stringify(result));
                ...
                console.log(items);
                var html = '<div class="section featureditem">';
                html    += '<p class="section-des">More powerful &amp; extensions from us. Please take a look from our extensions below!</p>';
                html    += '<ul class="magicslider software">';
                var placeholder = 'data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22' + '800' + '%22%20height%3D%22' + '600' + '%22%20viewBox%3D%220%200%20225%20265%22%3E%3C%2Fsvg%3E';
                $.each( items, function( key, item ) {
                    html += '<li class="item">';
                    html += '<div class="item-box">';
              ...
              proccessLazyload();
              var slidecfg = {"autoplay":true,"arrows":false,"autoplayspeed":1000,"dots":true,"infinite":true,"vertical":false,"verticalSwiping":false,"responsive":[{'breakpoint': 1200, 'settings': {'slidesToShow': 4}},{'breakpoint': 992, 'settings': {'slidesToShow': 3}},{'breakpoint': 768, 'settings': {'slidesToShow': 2}},{'breakpoint': 361, 'settings': {'slidesToShow': 1}}],"rows":1,"slidesToShow":4,"swipeToSlide":true,"speed":300};
              $('.magicslider.software').slick(slidecfg);
          }
       });
    })($); // End of use strict
});
